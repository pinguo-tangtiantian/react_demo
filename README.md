...
这个项目是由[Create React App](https://github.com/facebookincubator/create-react-app)引导生成的。

Below you will find some information on how to perform common tasks.<br>
You can find the most recent version of this guide [here](https://github.com/facebookincubator/create-react-app/blob/master/packages/react-scripts/template/README.md).
你将会在下面发现一些关于如何执行公共任务的信息。

## Table of Contents

- [升级到新的发行版](#updating-to-new-releases)
- [发送回调](#sending-feedback)
...
`create-react-app`是一个你用来创建新项目的使用的命令。

* `react-scripts` is a development dependency in the generated projects (including this one).
`react-scripts`是已生成文件（包括当前这个）中的一个开发依赖。

You almost never need to update `create-react-app` itself: it delegates all the setup to `react-scripts`.
你几乎永远不需要升级`create-react-app`本身：它会将所有设置都委托给`react-scripts`。

When you run `create-react-app`, it always creates the project with the latest version of `react-scripts` so you’ll get all the new features and improvements in newly created apps automatically.
当你运行`create-react-app`的时候，它总是会用最新版的`react-scripts`来创建项目，所以你将会在最新自动创建的项目中拥有所有最新的特性和和提升。


To update an existing project to a new version of `react-scripts`, [open the changelog](https://github.com/facebookincubator/create-react-app/blob/master/CHANGELOG.md), find the version you’re currently on (check `package.json` in this folder if you’re not sure), and apply the migration instructions for the newer versions.
要将一个已经存在的项目升级到最新版的`react-scripts`，[打开更新日志](https://github.com/facebookincubator/create-react-app/blob/master/CHANGELOG.md)，找到你当前的版本，并且想新版本申请迁移说明。

In most cases bumping the `react-scripts` version in `package.json` and running `npm install` in this folder should be enough, but it’s good to consult the [changelog](https://github.com/facebookincubator/create-react-app/blob/master/CHANGELOG.md) for potential breaking changes.
在大多数情况下，在这个目录的`package.json`中碰撞（？）`react-scripts`版本并且执行`npm install`就够了，但是查阅更新日志是一件好事。

We commit to keeping the breaking changes minimal so you can upgrade `react-scripts` painlessly.
我们承诺会尽量保持小的变化，这样你升级`react-scripts`的时候就不会太痛苦。
...
你也许会在`src`中创建子目录。为了更快的重建，只有`src`内部的文件才会被webpack加工。<br>
You need to **put any JS and CSS files inside `src`**, otherwise Webpack won’t see them.
你需要**将每一个JS文件和CSS文件都放在`src`目录里面**，否则webpack将会无视他们。


Only files inside `public` can be used from `public/index.html`.<br>
Read instructions below for using assets from JavaScript and HTML.
只有在`public`内部非文件才会被`public/index.html`使用</br>
阅读以下介绍来运用JavaScript 和 HTML。


You can, however, create more top-level directories.<br>
They will not be included in the production build so you can use them for things like documentation.
然而，你可以创建更多的顶层目录。</br>
他们将不会被包含在产品构建中，所以你可以用他们做一些事，比如记录。
...
在开发模式下运行程序</br>
在浏览器中打开http://localhost:3000来查看效果。

The page will reload if you make edits.<br>
You will also see any lint errors in the console.
如果你修改了文件，页面会重新加载。</br>
你也可以在控制台中看到任何lint错误。</br>
